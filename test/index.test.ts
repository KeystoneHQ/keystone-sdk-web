...
test('parseMultiAccounts without xpub', () => {
  ...
  expect(keystoneSDK.parseMultiAccounts(new UR(toBuffer(cborHex), type))).toStrictEqual(expectResult)
})
...
test('parseMultiAccounts throw error', () => {
  ...
  const parse = (): void => {
    keystoneSDK.parseMultiAccounts(new UR(toBuffer(cborHex), type))
  }
